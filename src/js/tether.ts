import { AttachmentPointState } from "./kite"
...
export class Tether {
  ...
  constructKiteTether(tp: TetherProperties) {
    ...
    this.pos.push() // new Vector3( tSegmentLength + dx, tetherAttachmentPoint1.y, 0 ))
    this.pos.push() // new Vector3( tSegmentLength + dx, tetherAttachmentPoint2.y, 0 ))
    this.vel.push() // new Vector3( 0, 0, 0 ))
    this.vel.push() // new Vector3( 0, 0, 0 ))
  ...
  updateTetherPositionAndForces(dt: number) {
    ...
    for (var i = 0; i <= this.KIndex2; i++) { // calculate forces for main and kite tethers
      var ap = C.WIND.clone().sub( this.vel[i] )
      ...
      var springConstant = tetherStretch > 0 ? this.tp.k0 : this.tp.k0_negative
